...
const MORNING_HOUR = 7;  // 朝の配信時刻(この時間帯のどこかで実行される)
const EVENING_HOUR = 20; // 夜の配信時刻(朝に配信済みの記事は自動で除外される)
...
function deliverNews(withSummary) {
	...
	let headerText = '📰 ' + formatDate(new Date()) + ' のニュース';
	const digest = buildDigest(aiArticles.concat(parentingArticles));
	if (digest) {
		headerText += '\n\n【今日のダイジェスト】\n' + digest;
	}
	const messages = [{ type: 'text', text: headerText }];
...
// --- Gemini API 呼び出しの共通部（キー未設定・エラー時は null） ---
...
// --- Geminiで記事を要約（キー未設定なら本文の先頭で代用） ---
function summarizeArticle(article) {
	const text = callGemini(`以下のニュース記事を日本語50文字程度で要約してください。要約文のみを返してください。

タイトル: ${article.title}
内容: ${article.description}`);
	return text || article.description.slice(0, 50);
}

// --- Geminiで「今日のダイジェスト」を生成（キー未設定なら空 → ヘッダーは日付のみ） ---
function buildDigest(articles) {
	if (articles.length === 0) return '';
	const list = articles.map(a => '・' + a.title).join('\n');
	const text = callGemini(`以下は今日配信するニュースのタイトル一覧です。全体を俯瞰して、今日の注目ポイントを日本語で3行以内の箇条書き（各行「・」始まり、1行40文字以内）にまとめてください。箇条書きのみを返してください。
...
// 朝夕2回の自動配信トリガーを設定する（一度実行すればOK。再実行すると設定し直し）
// 注意: このプロジェクトの既存トリガーをすべて置き換える
function setupTriggers() {
	ScriptApp.getProjectTriggers().forEach(t => ScriptApp.deleteTrigger(t));
	ScriptApp.newTrigger('dailyJobSimple').timeBased().everyDays(1).atHour(MORNING_HOUR).create();
	ScriptApp.newTrigger('dailyJobSimple').timeBased().everyDays(1).atHour(EVENING_HOUR).create();
	Logger.log('トリガー設定完了: 毎日 ' + MORNING_HOUR + '時台 と ' + EVENING_HOUR + '時台 に配信します');
	Logger.log('(要約版に切り替えたい場合は setupTriggers 内の dailyJobSimple を dailyJobWithSummary に変えて再実行)');
}
